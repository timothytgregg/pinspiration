def get_pin_data
  pin_data =
    {"ThingsThatFly"=>
      [{:title=>"First Flyer",
      :img_url=>"http://nyulocal.com/wp-content/uploads/2013/10/The-plane-flying-at-sunset-airliner-photography_1920x1080.jpg"},
      {:title=>"Second Flyer",
      :img_url=>"http://s7d2.scene7.com/is/image/Armstrong/ThingsThatFly_S705-755"}],
    "ThingsThatSwim"=>
      [{:title=>"First Swimmer",
      :img_url=>"http://www.defenders.org/sites/default/files/styles/large/public/dolphin-kristian-sekulic-isp.jpg"},
      {:title=>"Second Swimmer",
      :img_url=>"http://pad1.whstatic.com/images/thumb/f/f0/Draw-a-Shark-Step-8-Version-3.jpg/670px-Draw-a-Shark-Step-8-Version-3.jpg"}]
    }
  return pin_data
end
